...
| `test_country.py` | the one country setting: that your callsign decides it until you pick one, that changing it swaps the boundaries while the app is running (checked against what MapLibre is really holding, not only the globals), that the spots button, the count under the map and Settings all follow, that a selection from the country that just left is let go, and that a country without boundaries says so and keeps its points |
