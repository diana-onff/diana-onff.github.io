...
with sync_playwright() as p:
    ...
    ok(pg.evaluate("() => getComputedStyle(document.getElementById('instBar')).display")!="none", "bar visible with no panel open")
    pg.evaluate("() => select('ONFF-0953')"); pg.wait_for_timeout(600)
    ok(pg.evaluate("() => getComputedStyle(document.getElementById('instBar')).display")=="none", "bar gives way to an open panel")
    btn = pg.evaluate("""() => { const b=document.getElementById('zoneSpot').getBoundingClientRect();
                                 return {bottom:b.bottom, h:b.height}; }""")
    ok(btn["h"]>10 and btn["bottom"]<844, f"button in the panel fully usable again ({btn})")
    pg.screenshot(path="/tmp/shot_fixed.png")
    pg.evaluate("() => closeSheet()"); pg.wait_for_timeout(400)
    ok(pg.evaluate("() => getComputedStyle(document.getElementById('instBar')).display")!="none", "bar comes back on closing")
    br.close()
